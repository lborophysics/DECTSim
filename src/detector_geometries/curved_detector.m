classdef curved_detector < detector
    ...
    methods (Access=private, Static)
        function generator = get_ray_generator_static(ray_energy, ny_pixels, nz_pixels, pixel_angle, pixel_height, dist_to_detector, to_source_vec, source_pos, voxels)
            ...
            function [xray] = get_ray_attrs(y_pixel, z_pixel)
                ...
                xray = ray(source_pos, -pixel_vec, final_length, voxels, ray_energy);
            end
        end
    end

    methods
        function self = curved_detector(ray_source, sensor_unit, dist_to_detector, pixel_dims, n_pixels, num_rotations, scatter_type, scatter_factor)
            arguments
                ray_source      %source
                sensor_unit     %sensor
            ...
            self@detector(ray_source, sensor_unit, dist_to_detector, ...
                n_pixels, num_rotations, 2*pi, scatter_type, scatter_factor);
        ...
        function ray_generator = get_ray_generator(self, voxels, ray_energy, ray_per_pixel)
            ...
            ray_generator = self.get_ray_generator_static(...
                ray_energy, self.ny_pixels, self.nz_pixels, self.pixel_angle, ...
                self.pixel_height, self.dist_to_detector, self.to_source_vec, ...
                self.source_position, voxels...
            );
        end
        function pixel_generator = get_pixel_generator(self, angle_index, voxels, ray_energy, ray_per_pixel)
            % Create a function which returns the rays which should be fired to hit each pixel.
            % Only 1 ray per pixel is supported at the moment, as anti-aliasing techniques are not yet implemented.
            arguments
                self           curved_detector
            ...
            static_ray_generator = curved_detector.get_ray_generator_static(...
                ray_energy, self.ny_pixels, self.nz_pixels, self.pixel_angle, ...
                self.pixel_height, self.dist_to_detector, to_source_vec, ...
                source_pos, voxels...
            );
