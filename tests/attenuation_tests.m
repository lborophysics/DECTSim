classdef attenuation_tests < matlab.unittest.TestCase
    methods(Test)
        ...
        function test_constructor(tc)
            ...
            tc.verifyError(@() get_material(), 'MATLAB:minrhs', 'Not enough input arguments.');
            tc.verifyError(@() get_material("water", 2), 'MATLAB:invalidInput');
            tc.verifyError(@() get_material("1", 2, 3, 4, 5), 'MATLAB:invalidInput');
            tc.verifyError(@() get_material(1), 'assert:failure');
            tc.verifyError(@() get_material('water'), 'assert:failure');
            tc.verifyError(@() get_material("wat"), 'MATLAB:invalidMaterial');
            tc.verifyError(@() get_material("my_water", [1, 8], [0.111898, 0.888102], [0, 1]), 'assert:failure');
            tc.verifyError(@() get_material("my_water", [1, 8], [0.1, 0.2], "1"), 'assert:failure');
            tc.verifyError(@() get_material("my_water", [1, 2;1, 2], [0.111898, 0.888102], 1), 'assert:failure');
            tc.verifyError(@() get_material("my_water", [1, 8], [0.1, 0.2; 0.3, 0.4], 1), 'assert:failure');
            tc.verifyError(@() get_material("my_water", 1, [0.1, 0.2], 1), 'assert:failure');
        end
        ...
        function test_water(tc) % Don't forget to convet MeV to KeV
            ...
            water_mat = get_material("_water", [1, 8], [0.111898, 0.888102], 1);
            water = @(e) water_mat.get_mu(e*1000);
            tc.verifyEqual(water(1e-3  ), 4.078E+03, "RelTol", 1e-3)
            tc.verifyEqual(water(1.5e-2), 1.673E+00, "RelTol", 1e-3)
            tc.verifyEqual(water(8e-2  ), 1.837E-01, "RelTol", 1e-3)
